...
- **PCSX2 auto-load of the resume state (opt-in)**: a second switch under "Sync save states" in Settings → Emulators, "Auto-load resume state on launch" (off by default; disabled with "Not supported yet" for other emulators), makes PCSX2 boot straight into the game's resume state (`SERIAL (CRC).resume.p2s`, the newest one if several match) by adding `-statefile` to the launch arguments. It never loads a numbered quick-save slot, launches normally when there is no resume file, works with or without state sync, and applies to the headless CLI too. PCSX2 only writes the resume file when its save-state-on-shutdown option (the `SaveStateOnShutdown` setting) is on; Freegosy does not change PCSX2 settings.
